subroutine ssTauLeap(K,T,Y0,Y,options,info)
    !
    ...
    do while (i <= K)
        ...
        if ( fsolve_inf%iter >= default_fsolve_options%MaxIter ) then 
            !print*, i, fsolve_inf%iter
            !cycle
            !print*, '1)', a_tau_s
            !print*, '2)', dN
            goto 1000
    ...
    subroutine preserve_positivity()
        integer(kind=4) :: j, n
        !real(kind=8), dimension(num_reactions)  :: KK
        !KK = nint( dN + a_tau_p + a_tau_c - a_tau_s )
